...
static bool simplify_rational(ast_t *e) {
	...
	for(i = 0; i < ast_ChildLength(e); i++) {
		...
		if(child->type == NODE_OPERATOR) {
			...
			if(child->op.operator.type == OP_DIV) {
				...
				if(e->op.operator.type == OP_DIV) {
					...
					if(i == 0) {
						...
						ast_ChildInsert(e, ast_ChildRemoveIndex(child, 0), 0);
						ast_ChildAppend(e, ast_MakeBinary(OP_MULT, ast_ChildRemoveIndex(child, 0), ast_ChildRemoveIndex(e, 0)));
					...
					ast_ChildAppend(mult, e->op.operator.base);
